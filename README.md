...
1. **Fetch** — pulls remote US jobs from Greenhouse, Lever, Ashby, Recruitee, Workable, Rippling, and Breezy boards for every company in `config/companies.json` (async, ~1,100 companies in a few minutes). Extracts salary where available, detects hybrid/in-office postings, and archives stale listings.
